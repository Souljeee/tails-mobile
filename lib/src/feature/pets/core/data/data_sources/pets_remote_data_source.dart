...
class PetsRemoteDataSource {
  ...
  const PetsRemoteDataSource({required RestClient restClient}) : _restClient = restClient;
  ...
  Future<List<PetDto>> getPets() async {
    ...
    return response.map((e) => PetDto.fromJson(Map<String, dynamic>.from(e as Map))).toList();
  }
  ...
  Future<PetDetailsDto> getPetDetails({required int id}) async {
    ...
    return PetDetailsDto.fromJson(Map<String, dynamic>.from(response));
  }
  ...
  Future<void> addPet({required AddPetDto dto, required String? imagePath}) async {
    ...
    final response = await _restClient.multipart(
      '/pets/',
      fields: fields,
      files: [if (imagePath != null) RestClientMultipartFile.path(field: 'image', path: imagePath)],
    );
  ...
  Future<List<BreedDto>> getBreeds({required PetTypeEnum petType}) async {
    final response = await _restClient.get('/breeds/', queryParams: {'type': petType.name});
    ...
    return response.map((e) => BreedDto.fromJson(Map<String, dynamic>.from(e as Map))).toList();
  }
